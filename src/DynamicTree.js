...
export default class DynamicTree {
	...
	rotate(node) {
		...
		if (this.nodes.at(B).height === 0 && this.nodes.at(C).height > 0) {
			...
			const costBF = this.nodes.at(B).aabb.unionPerimeter(this.nodes.at(G).aabb)
			const costBG = this.nodes.at(F).aabb.unionPerimeter(this.nodes.at(B).aabb)
			...
			const costCE = this.nodes.at(D).aabb.unionPerimeter(this.nodes.at(C).aabb)
			const costCD = this.nodes.at(C).aabb.unionPerimeter(this.nodes.at(E).aabb)
		...
		if (grandParent !== null) {
			...
			while (ancestor !== null) {
				...
				this.nodes.at(ancestor).aabb = this.nodes
					.at(child1)
					.aabb.union(this.nodes.at(child2).aabb, this.nodes.at(ancestor).aabb)
				this.nodes.at(ancestor).height =
					1 +
